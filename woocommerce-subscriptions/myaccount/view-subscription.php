...
use  Inc\classes\front\controller\TemplateController;
...
wc_print_notices();
// var_dump($subscription->schedule_next_payment);
$nextdate = explode(" ", $subscription->schedule_next_payment)[0];
$diff = TemplateController::calcuate_date($nextdate);
// var_dump($diff);
?>
		...
		<tr>
			...
			<td>
				...
				?>
					<form method="POST">
						<label>Delay</label>
						<input type="date" name="delay_date" class="delay-date" require>
						<input type="submit" name="delay" value="Delay">
					</form>
				<?php
				}
				?>
...
<div class="clear"></div>
<script>
	let delay_date = document.querySelector('.delay-date').value = "<?php echo $nextdate; ?>";
</script>
<?php
$delay_date = $_POST['delay_date'];
if (isset($_POST['delay'])) {
	// echo $diff . "delay_date" . $delay_date;
	$delay_date = $delay_date . " " . date("h:i:s");
